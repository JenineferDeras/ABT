 ...
 */
async function requestFinancialDataset(
  signal?: AbortSignal,
): Promise<FinancialIntelligenceApiResponse> {
...
export function useMCPIntegration() {
  ...
  const productOpportunities = useMemo<ProductOpportunity[]>(
    () => state.dataset?.productOpportunities ?? [],
    [state.dataset],
  );

  const aiRunbooks = useMemo<AIRunbook[]>(
