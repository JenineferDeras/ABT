...
export default function ProductOpportunities({ opportunities, isLoading }: ProductOpportunitiesProps) {
    return (
        <section className="rounded-xl border border-purple-500/10 bg-gradient-to-br from-slate-900/70 via-slate-900/40 to-purple-950/30 p-6 shadow-xl">
            <header className="mb-6">
                <h3 className="text-xl font-semibold text-white">Product Opportunity Pipeline</h3>
                <p className="text-sm text-purple-200/80">
                    Quantified runway for strategic lending products with probability-weighted revenue and payback benchmarks.
                </p>
            </header>
                    ...
                    {opportunities.map((opportunity) => (
                        ...
                        >
                            <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                                <div>
                                    <p className="text-xs uppercase tracking-wide text-purple-300/80">{opportunity.segment}</p>
                                    <h4 className="text-lg font-semibold text-white">{opportunity.name}</h4>
                                </div>
                                <div className="flex flex-wrap items-center gap-2 text-xs">
                                    <span className={`rounded-full border px-3 py-1 font-medium ${stageStyles[opportunity.lifecycleStage]}`}>
                            ...
                            <p className="mt-3 text-sm text-slate-300/80">{opportunity.summary}</p>
                            ...
                            <div className="mt-5 grid grid-cols-1 gap-4 text-sm md:grid-cols-3">
                                <div className="rounded-lg border border-emerald-500/20 bg-emerald-500/10 p-3 text-emerald-100">
                                    <div className="text-xs uppercase tracking-wide text-emerald-300/80">Expected Revenue</div>
                                ...
                                <div className="rounded-lg border border-purple-500/20 bg-slate-950/40 p-3 text-purple-100">
                                    <div className="text-xs uppercase tracking-wide text-purple-300/80">Payback</div>
                                    <div className="mt-1 text-lg font-semibold text-white">{opportunity.paybackPeriodMonths} months</div>
                                </div>
                                <div className="rounded-lg border border-sky-500/20 bg-sky-500/10 p-3 text-sky-100">
                                    <div className="text-xs uppercase tracking-wide text-sky-300/80">Adoption Probability</div>
                                    <div className="mt-1 text-lg font-semibold text-white">
                                        {formatPercent(opportunity.adoptionProbability, { maximumFractionDigits: 0 })}
                                    </div>
                                </div>
                            </div>
                        </article>
